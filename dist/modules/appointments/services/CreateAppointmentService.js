...
let CreateAppointmentService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if ((0, _dateFns.isBefore)(appointmentDate, Date.now())) {
      throw new _AppError.default('You cannot book an appointment on a past date.');
    }

    if (user_id === provider_id) {
      throw new _AppError.default('You cannot book an appointment with yourself.');
    }

    if ((0, _dateFns.getHours)(appointmentDate) < 8 || (0, _dateFns.getHours)(appointmentDate) > 17) {
      throw new _AppError.default('You can only book an appointment between 8am and 17pm.');
    }
